<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-3.5 shadow-sm hover:shadow-md transition-shadow">
    ...
    <template v-else>
      ...
      <p class="text-xl font-bold text-gray-900 dark:text-white mb-1 tabular-nums">{{ value }}</p>
      <div v-if="change !== undefined" class="flex items-center gap-1">
        ...
        <span class="text-xs text-gray-400 dark:text-gray-500">vs prior</span>
      </div>
    </template>
  </div>
</template>
