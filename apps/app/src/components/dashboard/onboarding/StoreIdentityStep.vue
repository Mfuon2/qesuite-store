  ...
  <div class="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_280px]">
    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
      <section class="qs-card-soft p-3">
        <div class="mb-2">
          <h3 class="text-sm font-extrabold text-slate-950">Store basics</h3>
          <p class="text-xs font-medium text-slate-500">Name, category, and storefront address.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-2">
          ...
          <div class="sm:col-span-2">
            ...
            <div class="flex min-w-0 items-center">
              <span class="hidden h-9 items-center rounded-l-xl border border-r-0 border-[#d0daca] bg-emerald-50/70 px-3 text-xs font-bold text-emerald-800 sm:inline-flex">{{ storefrontHost }}/</span>
      ...
      <section class="qs-card-soft p-3 lg:col-span-2">
        <div class="mb-2">
          <h3 class="text-sm font-extrabold text-slate-950">Visual identity</h3>
          <p class="text-xs font-medium text-slate-500">Upload brand assets and choose colors.</p>
        </div>

        <div class="grid grid-cols-[minmax(0,1fr)_110px] gap-2 md:grid-cols-[minmax(0,1.4fr)_140px_minmax(220px,1fr)] md:gap-3">
          <div>
            <label class="admin-label">Store Banner</label>
            <ImageUpload ref="bannerUploadRef" :model-value="form.banner_url" compact @file-selected="uploadBanner" class="w-full" />
          </div>

          <div>
            <label class="admin-label">Store Logo</label>
            <ImageUpload ref="logoUploadRef" :model-value="form.logo_url" compact @file-selected="uploadLogo" />
          </div>

          <div class="col-span-2 grid grid-cols-2 content-start gap-2 md:col-span-1 md:grid-cols-1">
            <div>
      ...
      <section class="qs-card-soft p-3 lg:row-start-1 lg:col-start-2">
        <div class="mb-2">
          <h3 class="text-sm font-extrabold text-slate-950">Contact details</h3>
          <p class="text-xs font-medium text-slate-500">Where customers can reach or find you.</p>
        </div>
    ...
    <aside class="hidden xl:sticky xl:top-3 xl:block xl:self-start">
      <p class="mb-2 flex items-center gap-2 text-xs font-extrabold text-slate-600">
      ...
      <div class="overflow-hidden rounded-[1.35rem] border border-slate-100 bg-white shadow-[0_10px_32px_rgba(15,23,42,0.035)]">
        <div class="relative h-24 overflow-hidden bg-emerald-50" :style="{ backgroundColor: form.primary_color }">
        ...
        <div class="relative px-3 pb-3">
          <div class="-mt-7 flex items-end gap-2">
            <div class="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl border border-slate-100 bg-white shadow-[0_12px_28px_rgba(15,23,42,0.10)]">
              <img v-if="form.logo_url" :src="form.logo_url" class="h-full w-full object-cover" />
              <span v-else class="text-lg font-extrabold text-emerald-700">{{ (form.name || 'S')[0].toUpperCase() }}</span>
            </div>
            <div class="min-w-0 pb-1">
              <p class="truncate text-sm font-extrabold text-slate-950" :style="{ fontFamily: storeFontStack(form.font_family) }">{{ form.name || 'My Store' }}</p>
              <p class="truncate text-xs font-medium text-slate-500">{{ form.address || 'Store address' }}</p>
            </div>
          </div>
          <div class="mt-3 flex gap-1.5">
            <span class="rounded-full px-2 py-1 text-[10px] font-extrabold text-white" :style="{ backgroundColor: form.primary_color }">All Products</span>
            <span class="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-extrabold text-slate-500">Featured</span>
          </div>
          <div class="mt-3 grid grid-cols-2 gap-2">
            <div v-for="i in 2" :key="i" class="rounded-xl border border-slate-100 bg-slate-50 p-2">
